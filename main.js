...
function wpm() {
  ...
  const correctWords = typedWords.filter((word) => {
    ...
    const incorrectLetter = letters.filter((letter) =>
      letter.className.includes("incorrect")
    );
    const correctLetter = letters.filter((letter) =>
      letter.className.includes("correct")
    );
    return (
      incorrectLetter.length === 0 && correctLetter.length === letters.length
    );
  });
  return correctWords.length / 0.5;
}
...
function gameOver() {
  ...
  document.querySelector("#game").innerHTML = "Times up!";
...
document.getElementById("game").addEventListener("keyup", (ev) => {
  ...
  const isFirstWord = document.querySelector(".word.current").previousSibling;
  ...
  if (!window.timer && isLetter) {
    window.timer = setInterval(() => {
      ...
      const secPassed = 30 - Math.round(timePassed / 1000);
      if (secPassed <= 0) {
        gameOver();
        return;
      }
      document.getElementById("info").innerHTML = secPassed;
    }, 1000);
  }
  ...
  if (isBackspace) {
    ...
    if (!currentLetter) {
      ...
      // removeClass(currentWord.nextElementSibling.firstChild, "current");
      removeClass(currentWord.lastChild, "correct");
      removeClass(currentWord.lastChild, "incorrect");
      const remove = document.querySelectorAll(".extra");
      remove.forEach((rim) => rim.remove());
    }
  }
  ...
  cursor.style.top =
    (nextLetter || nextWord).getBoundingClientRect().top - 1 + "px";
  cursor.style.left =
